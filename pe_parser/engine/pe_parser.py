...
class PEParser:
    ...
    def _unpack_bytes(self, size: int, struct_format: str) -> Tuple[int, ...]:
    ...
    def _read_dos_header(self):
        ...
        e_lfanew = self._unpack_bytes(4, '<I')[0]
    ...
    def _read_file_header(self):
        ...
        self.file_header = FileHeader(
            pe_magic,
            *self._unpack_bytes(*self.FORMATS['IMAGE_FILE_HEADER']))
        logging.debug(msg='Read FileHeader')

    def _read_optional_header(self):
        magic = self._unpack_bytes(2, 'H')[0]
        if magic == 0x10b:
            self.pe_format = 'PE32'
        elif magic == 0x20b:
            self.pe_format = 'PE32+'
        ...
        self.optional_header_standard = \
            OptionalHeaderStandard(magic,
                                   *self._read_optional_header_standard())
        self.optional_header_windows_specific = OptionalHeaderWindowsSpecific(
            *self._read_optional_header_windows_specific()
        )
        ...
        for _ in range(
            ...
            self.optional_header_data_directories.append(
                DataDirectory(*self._unpack_bytes(
                    *self.FORMATS['DATA_DIRECTORY']
                )))
        logging.debug('Read data directories')

    def _read_optional_header_standard(self):
        return self._unpack_bytes(
            *self.FORMATS['OPTIONAL_HEADER_STANDARD'][self.pe_format]
        )

    def _read_optional_header_windows_specific(self):
        return self._unpack_bytes(
    ...
    def _read_sections(self):
        for _ in range(self.file_header.number_of_sections):
            section = SectionHeader(*self._unpack_bytes(
                *self.FORMATS['SECTION_HEADER']
            ))
            self.sections[section.name] = section
        logging.debug('Read sections')

    def _read_imported_dlls(self):
        ...
        import_descriptors = []
        while True:
            import_descriptor = ImportDescriptor(*self._unpack_bytes(
    ...
    @staticmethod
    def pe_file_block_to_pretty_dict(field) -> Dict[str, str]:
        block_dict = {}
